...
Class Master extends DBConnection {
	...
	function save_album(){
		...
		$data = "";
		$d1=$_POST['name'];
		$check = $this->conn->query("SELECT * FROM `album_list` where `name` = '{$d1}'")->num_rows;
		$this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
			...
			if(empty($id)){
				$sql = "INSERT INTO `album_list`( `name`, `user_id`, `delete_f`, `date_created`, `date_updated`) VALUES ('$d1','$_SESSION[user_id]','0','2021-08-09 10:23:50','2021-08-09 10:23:50')";
				$save = $this->conn->query($sql);
			}else{
				$sql = "UPDATE `album_list` set name={$d1} where id = {$id}";
				$save = $this->conn->query($sql);
			}
			$this->capture_err();
	...
	function delete_album(){
		$sql = "DELETE FROM  `album_list` where id = '{$_POST['id']}' ";
		$delete = $this->conn->query($sql);
		$this->capture_err();
		if($delete){
			$resp['status'] = 'success';
			$img_arch = $this->conn->query("DELETE FROM  `images`  where album_id = '{$_POST['id']}' ");
	...
	function retrieve_album(){
		...
		$this->capture_err();
		if($retrieve){
			$sql = "DELETE FROM  `images`  where album_id = '{$_POST['id']}' ";
