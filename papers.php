    ...
    <section class="content">
            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">All Papers with details</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
        ...
        $semester=$_POST["sem"];
                      ...
                      $sql = "SELECT prog,sub_code,subject_name,sess,semester,pdf_file FROM details WHERE prog='$prog' and sub_code='$sub_code' and sess='$sess' and semester=$semester";
$result = $conn->query($sql);
if($result){
if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
      ...
      $res="./private_html/private_a/dist/img_dir/".$res;
       echo "<tr><td>". $row["prog"]. "</td><td>". $row["sub_code"]."</td><td>" .$row["subject_name"]."</td><td>" . $row["sess"] . "</td><td>" . $row["semester"]."</td><td><a href='" .$res."'download>Paper : " .$row["subject_name"]."</a></td></tr>" ;
    }
