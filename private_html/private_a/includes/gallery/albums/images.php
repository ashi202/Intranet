...
<div class="w-100 d-flex justify-content-between border-bottom py-2">
	<h3><?php echo $name ?></h3>
	<a class="btn btn-flat btn-light border" href="../" >Admin</a>
    <div>
...
<div class="row row-cols-4 row-cols-md-3 row-cols-sm-1 row-cols-lg-4 py-2">
	<?php 
		$qry = $conn->query("SELECT * FROM `images` where user_id = '{$_SESSION['user_id']}' and `delete_f` = 0 and album_id = '{$_GET['id']}' order by `original_name` asc ");
